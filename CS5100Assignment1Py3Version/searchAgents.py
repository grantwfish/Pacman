        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not hitsWall:
                if currentPosition not in corners:
                    ...
                    nextSpot = ((nextx, nexty), tuple(newCorners))

                successors.append((nextSpot, action, 1))
...
def foodHeuristic(state, problem):
    ...
    # for chow in foodList:
    #     if util.manhattanDistance(position, chow) > distanceToFood:
    #         distanceToFood = util.manhattanDistance(position, chow)
    # return distanceToFood
    # 6.8 sec and 9551 nodes = 3/4 :(

    for chow in foodList:

        distance = mazeDistance(position, chow, problem.startingGameState)
        if distance > distanceToFood:
            distanceToFood = distance
    return distanceToFood

    # 34-40 sec 4140 nodes :)
